...
/**
 * Issue Detail Page - Connected to Real API
 *
...
export const IssueDetail: React.FC = () => {
  ...
  const { data: issue, isLoading, isError } = useIssue(issueKey || '');
  ...
  // Update local description when issue loads
  React.useEffect(() => {
  ...
  const handleDeleteIssue = () => {
    if (issue && window.confirm(`Are you sure you want to delete ${issue.key}?`)) {
      deleteIssueMutation.mutate(issue.key, {
        onSuccess: () => {
          navigate('/board');
        },
      });
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-neutral-600">Loading issue...</p>
        </div>
      </div>
    );
  }
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex gap-3 text-sm">
                      <Avatar src={issue.reporter?.avatar} alt={issue.reporter?.name || 'Reporter'} size="sm" />
                      <div className="flex-1">
                        <p className="text-neutral-700">
                          <span className="font-semibold">{issue.reporter?.name || 'Reporter'}</span> created this
                          issue
                        </p>
                    ...
                    {issue.assignee && (
                      <div className="flex gap-3 text-sm">
                        ...
                        <div className="flex-1">
                          <p className="text-neutral-700">
                            <span className="font-semibold">{issue.reporter?.name || 'Reporter'}</span> assigned
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <Avatar
                        ...
                        size="sm"
                      />
                      <span className="text-sm text-neutral-900">{issue.reporter?.name || 'Unknown'}</span>
                    </div>
                  </div>
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-neutral-900">
                      {issue.estimatedHours ? `${issue.estimatedHours}h` : 'Not set'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-neutral-900">
                      {issue.estimatedHours && issue.loggedHours
                        ? `${issue.estimatedHours - issue.loggedHours}h`
                        : 'N/A'}
                    </span>
                  </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg flex items-center gap-2">
                    ...
                    Attachments ({issue.attachmentCount || 0})
                  </CardTitle>
                </CardHeader>
                <CardContent>
